<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="好物类型" prop="haowuleixing">
          <el-select v-model="ruleForm.haowuleixing" placeholder="请选择好物类型">
            <el-option
                v-for="(item,index) in haowuleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="好物类型" prop="haowuleixing">
	      <el-input v-model="ruleForm.haowuleixing"
                placeholder="好物类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                              </el-row>
  ...
  data() {
	...
	haowuleixing : false,
            ...
            ruleForm: {
                    ...
                    rules: {
                  haowuleixing: [
                            { required: true, message: '好物类型不能为空', trigger: 'blur' },
                                    	                                                              ],
              }
    };
  },
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='haowuleixing'){
            this.ruleForm.haowuleixing = obj[o];
	    this.ro.haowuleixing = true;
            continue;
          }
	                    }
                              }
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `haowuleixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.haowuleixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
